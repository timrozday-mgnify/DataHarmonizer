...
class Toolbar {
  ...
  bindEvents() {
    ...
    $('#show-recommended-only-cols-dropdown-item').on(
      'click',
      this.showRecommendedOnlyColumns.bind(this)
    );
    $(this.root).on(
      'click',
  ...
  // Builds {schema, location, version, in_language, Container} from the
  // current grid data (this.context.dhs) — the same shape saveFile() writes
  // to a JSON file, but as an in-memory object with no DOM/Blob dependency.
  // Extracted from saveFile() so it can also back getExportJson() below.
  buildExportJson(lang = 'default') {
  ...
  // Empty-key filtering applied to a JSONFormat.Container before JSON export
  // (drops keys with empty-string values per entry). Extracted from
  // saveFile()'s `ext === 'json'` branch so getExportJson() can share it.
  _cleanContainerForJson(Container) {
    ...
    const cleaned = {};
    for (let concept in Container) {
      cleaned[concept] = processEntryKeys(Container[concept]);
    }
    return cleaned;
  }

  // Programmatic export API: the same data saveFile() would write to a JSON
  // file, returned in-memory instead of going through a Blob/download. Used
  // by embedding pages (see window.dataHarmonizer in web/index.js) that want
  // the current grid data without driving the Save As dialog.
  getExportJson(lang = 'default') {
    const JSONFormat = this.buildExportJson(lang);
    JSONFormat.Container = this._cleanContainerForJson(JSONFormat.Container);
    return JSONFormat;
  }

  // Programmatic load API: the in-memory inverse of getExportJson(). Takes the
  // same {Container: {ClassName: [title-keyed rows]}} shape and populates the
  // grid(s), without a File/Blob. Mirrors openFile()'s JSON dispatch loop. Used
  // by embedding pages (see window.dataHarmonizer) to restore saved grid data.
  loadExportJson(jsonObj, locale = null) {
    if (!jsonObj || !jsonObj.Container) return;
    const containerSchema = this.context.template.default.schema.classes.Container;
    for (const dh in this.context.dhs) {
      const container_class = rangeToContainerClass(containerSchema, dh);
      const rows = jsonObj.Container[container_class];
      if (!rows) continue;
      const list_data = this.context.dhs[dh].loadDataObjects(rows, locale);
      if (list_data) {
        this.context.dhs[dh].hot.loadData(list_data);
        // getExportJson() bakes formula(...) cells down to plain text on
        // export; restore them as live formulas so they keep recomputing
        // (see DataHarmonizer.js: _reapplyFormulaCells for why this can't
        // just be _populateDefaultValues).
        this.context.dhs[dh]._reapplyFormulaCells();
      }
    }
  }

  // --- Cell-level API (single-class templates) -----------------------------
  // Lower-level than getExportJson/loadExportJson: read/write individual
  // cells by column *title* and upsert a row by a key column without
  // touching any other column on that row. Used by embedding pages to sync
  // a small subset of fields (e.g. a name+sample pairing) into a grid whose
  // other columns may already hold user- or default-filled values that must
  // not be clobbered (unlike loadExportJson's full-row replace).

  getCurrentDh() {
    return this.context.getCurrentDataHarmonizer();
  }

  getCellValue(rowIndex, colTitle) {
    const dh = this.getCurrentDh();
    const col = dh.getFieldYCoordinates()[colTitle];
    return col === undefined ? undefined : dh.hot.getDataAtCell(rowIndex, col);
  }

  setCellValue(rowIndex, colTitle, value) {
    const dh = this.getCurrentDh();
    const col = dh.getFieldYCoordinates()[colTitle];
    if (col === undefined) return false;
    dh.hot.setDataAtCell(rowIndex, col, value, 'api_set');
    return true;
  }

  getRowCount() {
    return this.getCurrentDh().hot.countSourceRows();
  }

  findRowIndex(keyColTitle, keyValue) {
    const dh = this.getCurrentDh();
    const col = dh.getFieldYCoordinates()[keyColTitle];
    if (col === undefined) return -1;
    const total = dh.hot.countSourceRows();
    for (let row = 0; row < total; row++) {
      if (String(dh.hot.getDataAtCell(row, col) ?? '') === String(keyValue)) return row;
    }
    return -1;
  }
  ...
  addRow() {
    const dh = this.getCurrentDh();
    const row = dh.hot.countSourceRows();
    // addRows() (not a raw hot.alter) so foreign-key parent values and
    // ifabsent defaults get populated the same way the UI's own "insert row"
    // does.
    dh.addRows('insert_row_below', 1, row);
    return row;
  }

  // Find-or-create a row by keyColTitle, then set only the given columns —
  // any other column already present on that row (e.g. PLATFORM filled in
  // manually or via an ifabsent default) is left untouched.
  upsertRow(keyColTitle, keyValue, valuesByColTitle) {
    return this.upsertRows(keyColTitle, [
      { key: keyValue, values: valuesByColTitle },
    ])[0];
  }

  // Batched upsertRow: `entries` is [{key, values}, …] against one key column.
  // Called in a loop, upsertRow costs a full key-column scan, a render and a
  // change/validation pass *per row*; an embedder syncing a whole table paid
  // that N times over. Here the key column is scanned once, and all the cell
  // writes go in as a single setDataAtCell inside one batchRender. Returns
  // the row index of each entry, in order.
  //
  // batchRender (not batch): rows may be inserted as we go, and the row
  // indices we hand to setDataAtCell have to reflect those inserts
  // immediately — only rendering is safe to defer here.
  upsertRows(keyColTitle, entries) {
    const dh = this.getCurrentDh();
    const coords = dh.getFieldYCoordinates();
    const keyCol = coords[keyColTitle];
    if (keyCol === undefined) return (entries || []).map(() => -1);

    const rowByKey = new Map();
    const total = dh.hot.countSourceRows();
    for (let row = 0; row < total; row++) {
      rowByKey.set(String(dh.hot.getDataAtCell(row, keyCol) ?? ''), row);
    }

    return dh.hot.batchRender(() => {
      const changes = [];
      const rows = (entries || []).map(({ key, values }) => {
        let row = rowByKey.get(String(key));
        if (row === undefined) {
          row = this.addRow();
          rowByKey.set(String(key), row);
        }
        changes.push([row, keyCol, key]);
        Object.entries(values || {}).forEach(([title, value]) => {
          const col = coords[title];
          if (col !== undefined) changes.push([row, col, value]);
        });
        return row;
      });
      if (changes.length) dh.hot.setDataAtCell(changes, null, null, 'api_set');
      return rows;
    });
  }
  ...
  showRecommendedOnlyColumns() {
    this.toggleColumnVisibility('showRecommendedOnlyColumns');
  }
  ...
  toggleColumnVisibility(method, arg) {
    const showColsSelectors = [
      ...
      '#show-recommended-only-cols-dropdown-item',
      '.show-section-dropdown-item',
    ];
