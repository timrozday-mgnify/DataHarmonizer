...
class Toolbar {
  ...
  upsertRow(keyColTitle, keyValue, valuesByColTitle) {
    let row = this.findRowIndex(keyColTitle, keyValue);
    if (row === -1) row = this.addRow();
    this.setCellValue(row, keyColTitle, keyValue);
    Object.entries(valuesByColTitle || {}).forEach(([title, value]) =>
      this.setCellValue(row, title, value)
    );
    return row;
  }
