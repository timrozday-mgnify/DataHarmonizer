...
class DataHarmonizer {
  ...
  /**
   * Translate {slot_name} tokens in a formula template to A1-notation for the
   * given Handsontable row (0-based). Unknown slot names are left unchanged.
   * e.g. _buildFormula('=CONCATENATE({alias}, "_", {locus_name})', 3)
   *      → '=CONCATENATE(C4, "_", D4)'  (if alias=col2, locus_name=col3)
   */
  _buildFormula(formulaTemplate, row) {
    const hfRow = row + 1; // HyperFormula rows are 1-indexed
    return formulaTemplate.replace(/\{([^}]+)\}/g, (match, slotName) => {
      const col = this.slot_name_to_column[slotName];
      // slot_name_to_column values may be strings; coerce to number before column conversion
      const colNum = col !== undefined ? Number(col) : undefined;
      return colNum !== undefined && !isNaN(colNum) ? this._colIndexToLetter(colNum) + hfRow : match;
    });
  }
