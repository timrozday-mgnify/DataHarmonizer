...
class DataHarmonizer {
      ...
      */
      const hot_settings = {
        ...
        beforeChange: function (grid_changes, action) {
          ...
          // Clear default-value tracking for any cell being manually edited
          for (const [row, col] of grid_changes) {
            if (self.default_cells[row]) {
        ...
        afterRenderer: (TD, row, col) => {
          ...
          if (self.default_cells[row]?.has(col)) {
      ...
      this._populateDefaultValues(0, this.hot.countRows());
    } else {
      console.warn(
  ...
  /**
   * Populates newly added rows with ifabsent default values and marks
   * those cells in default_cells so they receive distinct shading.
   */
  _populateDefaultValues(startRowIndex, numRows) {
    ...
    if (changes.length) {
      // Set cell values first — beforeChange fires here and must not clear default_cells
      this.hot.setDataAtCell(changes, 'add_row');
      // Mark defaults only AFTER setDataAtCell returns (so beforeChange can't delete them)
