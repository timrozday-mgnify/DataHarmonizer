...
import { parseIfabsent, colIndexToLetter, buildFormula } from './utils/formulas';
...
class DataHarmonizer {
  ...
  invalid_cells = null;
  default_cells = {}; // { [row]: Set<col> } tracks cells holding an ifabsent default
  ...
  constructor(root, context, options = {}) {
    ...
    if (!$('#specify-headers-modal').length) $(this.modalsRoot).append(specifyHeadersModal);
    if (!$('#unmapped-headers-modal').length) $(this.modalsRoot).append(unmappedHeadersModal);
    if (!$('#field-description-modal').length) $(this.modalsRoot).append(fieldDescriptionsModal);
      ...
      */
      const hot_settings = {
        ...
        beforeChange: function (grid_changes, action) {
          ...
          // Clear default tracking for any cell being manually edited
          for (const [row, col] of grid_changes) {
            self.default_cells[row]?.delete(col);
          }
        ...
        afterRenderer: (TD, row, col) => {
          ...
          const rawVal = self.hot.getSourceDataAtCell(row, col);
          $(TD).removeClass('formula-cell default-value-cell');
          if (typeof rawVal === 'string' && rawVal.startsWith('=')) {
            $(TD).addClass('formula-cell');
          } else if (self.default_cells[row]?.has(col)) {
            $(TD).addClass('default-value-cell');
          }
        },
      ...
      // Use setTimeout(0) so the FormulasPlugin finishes all synchronous hook processing from
      // updateSettings before we interact with it. Capture the current hot instance so a stale
      // setTimeout callback from a prior createHot() call doesn't fire on the newer instance.
      const capturedHot = this.hot;
      setTimeout(() => {
        if (this.hot !== capturedHot || !capturedHot) return; // stale callback — newer hot was created
        const fp = capturedHot.getPlugin('formulas');
        if (fp?.enabled && fp.sheetId === null) {
          // onAfterLoadData never ran (plugin enabled after data load) — set up the sheet now.
          const data = capturedHot.getSourceDataArray();
          const sheetName = fp.addSheet(undefined, data);
          if (sheetName !== false) {
            fp.sheetName = sheetName;
            fp.indexSyncer.setupSyncEndpoint(fp.engine, fp.sheetId);
          }
        }
        this._populateDefaultValues(0, capturedHot.countRows());
      }, 0);
    } else {
      console.warn(
  ...
  addRows(row_where, numRows, startRowIndex = false) {
    ...
    if (!parents) {
      ...
      this._populateDefaultValues(startRowIndex, numRows);
      return;
    }
   ...
   * Returns null for formula(...) expressions (handled separately).
   */
  _parseIfabsent(ifabsent) {
    return parseIfabsent(ifabsent);
  }

  _colIndexToLetter(col) {
    return colIndexToLetter(col);
  }

  _buildFormula(formulaTemplate, row) {
    return buildFormula(formulaTemplate, row, this.slot_name_to_column);
  }

  /**
   * Populates newly added rows with ifabsent default/formula values.
   * formula(...) ifabsent values are inserted as live HyperFormula expressions.
   */
  _populateDefaultValues(startRowIndex, numRows) {
    const changes = [];
    const toMarkDefault = [];
    for (let row = startRowIndex; row < startRowIndex + numRows; row++) {
      this.slots.forEach((field, col) => {
        if (field.ifabsent?.startsWith('formula(')) {
          const template = field.ifabsent.slice(8, -1); // strip 'formula(' and ')'
          changes.push([row, col, this._buildFormula(template, row)]);
        } else {
          const def = this._parseIfabsent(field.ifabsent);
          if (def !== null) {
            changes.push([row, col, def]);
            toMarkDefault.push([row, col]);
          }
        }
      });
    }
    if (changes.length) {
      // Use 4-arg form so 'add_row' is the actual source param (4th arg).
      // setDataAtCell(array, source) makes source the column (2nd arg), leaving
      // the actual source undefined → beforeChange fires with 'edit' instead.
      this.hot.setDataAtCell(changes, null, null, 'add_row');
      // Mark only AFTER setDataAtCell returns (so beforeChange can't delete them)
      for (const [row, col] of toMarkDefault) {
        if (!this.default_cells[row]) this.default_cells[row] = new Set();
        this.default_cells[row].add(col);
      }
      setTimeout(() => this.hot.render(), 0);
    }
  }

  /**
   * Re-establishes live formula(...) cells across every existing row (not
   * just newly-added ones). Needed after hot.loadData() — unlike addRows(),
   * loadData() doesn't go through _populateDefaultValues, and getExportJson()
   * (fullData()) bakes formula cells down to their currently-computed plain
   * text for export, so without this, a save/reload round-trip permanently
   * freezes every formula cell at whatever value it had when last exported.
   * Deliberately narrower than _populateDefaultValues: only formula(...)
   * fields are touched (plain ifabsent defaults are left alone, since
   * reapplying those on already-loaded rows would overwrite real edited
   * values, not just recompute a live expression).
   */
  _reapplyFormulaCells() {
    const total = this.hot.countSourceRows();
    const changes = [];
    this.slots.forEach((field, col) => {
      if (!field.ifabsent?.startsWith('formula(')) return;
      const template = field.ifabsent.slice(8, -1);
      for (let row = 0; row < total; row++) {
        changes.push([row, col, this._buildFormula(template, row)]);
      }
    });
    if (changes.length) {
      this.hot.setDataAtCell(changes, null, null, 'reload_formula');
      setTimeout(() => this.hot.render(), 0);
    }
  }
  ...
  showRecommendedOnlyColumns() {
    this._updateVisibility((field) => !field.recommended);
  }
...
<body>
  ...
  getColumns() {
    ...
    for (let field of fields) {
      ...
      col.source = null;
      if (field.sources) {
        col.source = this.updateSources(field);
  ...
  */
  ...
  fullData(handsontableInstance = null) {
    ...
    // Resolve formula cells to their computed values for export
    const formulasPlugin = handsontableInstance.getPlugin('formulas');
    if (formulasPlugin?.enabled) {
      return array.map((row, r) =>
        row.map((cell, c) =>
          typeof cell === 'string' && cell.startsWith('=')
            ? handsontableInstance.getDataAtCell(r, c)
            : cell
        )
      );
    }
